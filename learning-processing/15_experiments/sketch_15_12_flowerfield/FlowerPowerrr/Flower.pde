...
class Flower { 
  ...
  float fx = constrain(x, 0, 200); // flower x constrain

  // The Constructor is defined with parameters.  
  Flower(int tempX, int tempY, float tempFlowerHeight, int tempNumDots, int tempNumDots2, int tempNumDots3, int tempBodySize, color tempYellow, color tempGreen, color tempOrange) { 
    x = tempX;
    y = tempY; 
